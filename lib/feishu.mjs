...
const FEISHU_DEFAULT_PICKER_ICON_TITLE = 'AppDefaultFilled';
const FEISHU_DEFAULT_PICKER_COLOR_STYLE_FRAGMENT = 'rgb(101, 111, 236)';
...
function getPageUrlSafely(page) {
  try {
...
async function ensureOpenPlatformPage(page) {
  const listResponsePromise = suppressUnhandledRejection(page.waitForResponse((response) => {
  ...
  try {
    ...
    throw new Error(
      `打开飞书开放平台失败，当前 URL: ${getPageUrlSafely(page)}。这通常表示登录态失效、页面被重定向，或飞书开放平台接口行为已变化。原始错误: ${formatErrorMessage(error)}`,
      { cause: error },
    );
  }
}
...
async function browserFormDataFetch(page, url, options = {}) {
  ...
  return await page.evaluate(async ({ base64, headers, referrer, url: requestUrl }) => {
    ...
    formData.append('file', new Blob([bytes], { type: 'image/png' }), 'bot-avatar.png');
...
async function waitForLogin(page, onQr, onLog) {
  ...
  const recoverQrFlow = async (message) => {
    const now = Date.now();
    if (now - lastRecoveryAt < 8_000) {
      return;
    }
    lastRecoveryAt = now;
    lastQrSignalAt = now;
  ...
  const onResponse = async (response) => {
    ...
    if (status === 5) {
      await recoverQrFlow('二维码已过期，正在刷新登录页。');
      return;
    }
  ...
  const qrProbeLoop = (async () => {
    while (!settled) {
      ...
      if (!lastQrDataUrl && Date.now() - lastQrSignalAt > 8_000) {
        await recoverQrFlow('长时间未拿到二维码，正在刷新登录页重试。');
      }
      await sleep(1500);
    }
  })();
...
async function openFeishuCreateAppDialog(page) {
  const dialog = page.locator('.create-app-dialog').first();
  const isVisible = await dialog.isVisible().catch(() => false);
  if (!isVisible) {
    await page.getByRole('button', { name: '创建企业自建应用' }).click();
  }
  await dialog.waitFor({ state: 'visible', timeout: 15_000 });
  await dialog.locator('.app-icon-picker').waitFor({ state: 'visible', timeout: 15_000 });
  return dialog;
}

async function closeFeishuCreateAppDialog(page) {
  const dialog = page.locator('.create-app-dialog').first();
  if (!(await dialog.isVisible().catch(() => false))) {
    return;
  }

  const closeButton = dialog.locator('button.ud__modal__close').first();
...
async function uploadGeneratedAvatarFromPickerInBrowser(page, reusableHeaders) {
  const dialog = await openFeishuCreateAppDialog(page);

  try {
    const preferredColor = dialog.locator(`.app-icon-picker__color-item[style*="${FEISHU_DEFAULT_PICKER_COLOR_STYLE_FRAGMENT}"]`).first();
    if (await preferredColor.count()) {
      await preferredColor.click();
    } else {
      const colorItems = dialog.locator('.app-icon-picker__color-item');
      const colorCount = await colorItems.count();
      if (colorCount) {
        await colorItems.nth(Math.min(colorCount - 1, 7)).click();
      }
    }

    const preferredIcon = dialog.locator(`.app-icon-picker__icons span[title="${FEISHU_DEFAULT_PICKER_ICON_TITLE}"]`).first();
    if (await preferredIcon.count()) {
      await preferredIcon.click();
    } else {
      const iconItems = dialog.locator('.app-icon-picker__icons .cursor-pointer');
      const iconCount = await iconItems.count();
      if (iconCount) {
        await iconItems.first().click();
      }
    }

    await page.waitForTimeout(200);

    const previewSrc = (await dialog.locator('.app-icon-picker__left .ud__avatar__image').getAttribute('src'))?.trim() || '';
    const base64 = extractBase64PayloadFromDataUrl(previewSrc);
    if (!base64) {
      return '';
    }
    ...
    await closeFeishuCreateAppDialog(page);
  }
}
...
export async function createFeishuBot(auth, options = {}) {
  ...
  const result = await withFeishuBrowserSession(auth, async ({ page, reusableHeaders }) => {
    onLog?.('使用飞书内置背景色和图标生成应用图标。');
    const avatar = await uploadGeneratedAvatarFromPickerInBrowser(page, reusableHeaders).catch(() => '');
    if (!avatar) {
      throw new Error('飞书应用图标生成失败：无法从图标选择器生成头像。');
    }
...
async function loadSuiteAppDetailInBrowser(page, suiteOrigin, appId) {
  const detailResponsePromise = suppressUnhandledRejection(page.waitForResponse((response) => {
  ...
  try {
    ...
    throw new Error(
      `打开飞书管理员后台应用详情失败，appId=${appId}，当前 URL: ${getPageUrlSafely(page)}。原始错误: ${formatErrorMessage(error)}`,
      { cause: error },
    );
  }
}
