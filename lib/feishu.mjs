...
export async function getFeishuBotSecrets(auth, appIds = []) {
  ...
  return await withFeishuBrowserSession(auth, async ({ page, reusableHeaders }) => {
    const secrets = {};

    for (const appId of uniqueIds) {
      try {
        secrets[appId] = await getFeishuAppSecretInBrowser(page, reusableHeaders, appId);
      } catch {
        secrets[appId] = '';
      }
    }
...
export async function getFeishuOpenApps(auth, sceneTypes = [0]) {
  const targetSceneTypes = Array.from(new Set(
    (Array.isArray(sceneTypes) ? sceneTypes : [sceneTypes])
      .map((item) => Number(item))
      .filter((item) => Number.isInteger(item) && item >= 0),
  ));

  if (!targetSceneTypes.length) {
    return [];
  }

  return await withFeishuBrowserSession(auth, async ({ page, reusableHeaders }) => {
    const items = [];
    for (const sceneType of targetSceneTypes) {
      items.push(...await listAllOpenAppsInBrowser(page, reusableHeaders, sceneType).catch(() => []));
    }
    return uniqByAppId(items);
  });
}
