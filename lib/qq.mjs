...
async function uploadDefaultAvatar(auth, avatarAsset = null) {
  ...
  formData.set('file', new Blob([asset.buffer], { type: asset.mimeType }), asset.fileName);
  formData.set('type', '0');
...
export async function authorizeQq({ onLog, onQr } = {}) {
  ...
  try {
    ...
    onQr?.({
      dataUrl: login.qrcodeUrl,
      expiresAt: new Date(qrIssuedAt + QQ_QR_TTL_MS).toISOString(),
      refreshCount: qrRefreshCount,
    });

    const deadline = Date.now() + LOGIN_TIMEOUT_MS;
    let developerId = '';
    let didReportScanned = false;
    ...
    while (Date.now() < deadline) {
      ...
      const shouldRefresh = code === QQ_POLL_EXPIRED
        || code === QQ_POLL_REJECTED
        || (code !== QQ_POLL_SCANNED && Date.now() - qrIssuedAt >= QQ_QR_TTL_MS);
      ...
      if (code === QQ_POLL_SCANNED && !didReportScanned) {
        didReportScanned = true;
...
export async function createQqBotWithProfile(auth, options = {}) {
  ...
  const avatarUpload = await uploadDefaultAvatar(auth, avatarAsset).catch((error) => {
  ...
  await modifyQqBotProfile(auth, appId, name, description, avatarUpload.avatar).catch(() => {});
